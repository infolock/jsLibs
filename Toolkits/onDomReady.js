...
(function() {
  var isBound = false, readyList = [], w = window, d = document, delm = d.documentElement,
  onDomReadyIdentifier = (function() {
    var identifier = 'onDomReady';
    if (w[identifier] && typeof w[identifier] == 'function') {
        return;
    }
    return identifier;
  })(),
  ...
  bindReady = function() {
    ...
    if (d.addEventListener) {
      ...
      d.addEventListener("DOMContentLoaded", DOMContentLoaded, false);
      w.addEventListener("load", whenReady, false); // fallback
      ...
      w.attachEvent("onload", whenReady); // fallback
  ...
  w[onDomReadyIdentifier] = function(callback) {
